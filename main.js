...
const getData100 = async() => {
    url = new URL(`${MOUNTAINS_URL}B553662/top100FamtListBasiInfoService/getTop100FamtListBasiInfoList?type=json&serviceKey=${MOUNTAINS_KEY}`)
    const response = await fetch(url);
    const data = await response.json();
    console.log(data)
}
